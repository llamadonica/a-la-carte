...
abstract class ALaCartePageCommon extends PolymerElement {
  ...
  void trackPage(ev) {
    ...
    for (;
        prev != null && prev.nodeName != 'SECTION';
    ...
    if (next == null && track < 0) {
      track =
          (60 / Math.PI * Math.atan(track.toDouble() * Math.PI / 60)).ceil();
      ...
      track =
          (60 / Math.PI * Math.atan(track.toDouble() * Math.PI / 60)).floor();
  ...
  void trackEndPage(ev) {
    ...
    (event['target'] as Node).childNodes.forEach((div) {
      ...
      div.style.transition =
          'all ${NOMINAL_SWITCH_DURATION}s cubic-bezier($coordB, $coordA, 1, 1) 0s';
    });
    ...
    if (next == null && track < 0) {
      var trackOffset = track.toDouble() -
          (60 / Math.PI * Math.atan(track.toDouble() * Math.PI / 60));
      ...
      (event['target'] as Node).childNodes.forEach((div) {
        if (div is! DivElement) return;
        div.style.transition =
            'all ${trackTime}s cubic-bezier($coordB, $coordA, 1, 1) 0s';
      });
    } else if (prev == null && track > 0) {
      var trackOffset = track.toDouble() -
          (60 / Math.PI * Math.atan(track.toDouble() * Math.PI / 60));
